...
// Visor de PDF basado en pdf.js (renderiza a <canvas>, funciona en movil real
// donde los navegadores no tienen visor de PDF nativo dentro de un iframe)
const PDFJS_VERSION = '4.0.379'
function PdfCanvasViewer({ data }: { data: Uint8Array }) {
  ...
  useEffect(() => {
    ...
    const cargarScript = (src: string) => new Promise<void>((resolve, reject) => {
    ...
    const render = async () => {
      try {
        if (!(window as any).pdfjsLib) {
          await cargarScript(`https://cdnjs.cloudflare.com/ajax/libs/pdf.js/${PDFJS_VERSION}/pdf.min.js`)
        }
        ...
        const escala = Math.min(2, Math.max(1.2, (window.devicePixelRatio || 1) * 1.1))
        ...
        if (!cancelado) { setError(e?.message || 'Error mostrando el PDF'); setCargando(false) }
      }
    }
  ...
  return (
    <div style={{ flex: 1, overflow: 'auto', background: '#525659', padding: '16px 12px' }}>
      {cargando && !error && (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10, padding: 40, color: '#fff', fontFamily: 'var(--mono)', fontSize: 12 }}>
          <span className="spinner" />
          Cargando PDF...
        </div>
      )}
      {error && (
        <div style={{ padding: 20, color: '#fca5a5', fontFamily: 'var(--mono)', fontSize: 12, textAlign: 'center' }}>
          ⚠ {error}
        </div>
      )}
...
{/* - Modal formulario tratamiento - */}
      ...
      {fichaVisible && fitoSeleccionado?.pdf_url && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 9000, display: 'flex', flexDirection: 'column', background: 'rgba(0,0,0,0.92)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 16px', background: 'var(--surface)', borderBottom: '1px solid var(--border)', flexShrink: 0 }}>
            <span style={{ fontFamily: 'var(--mono)', fontSize: 12, fontWeight: 700, color: 'var(--green)' }}>
              Ficha tecnica: {fitoSeleccionado.nombre}
            </span>
            <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
              ...
              <button
                onClick={() => { setFichaVisible(false); if (pdfBlobUrl) { URL.revokeObjectURL(pdfBlobUrl); setPdfBlobUrl(null) }; setPdfData(null) }}
          ...
          {pdfData
            ...
            : <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontFamily: 'var(--mono)', fontSize: 12 }}>Cargando PDF...</div>
          }
        </div>
      )}
    </div>
